...
AppController::AppController(App::Views::MainWindow* main,
    ...
    QObject::connect(main_->landing_page(), &App::Views::LandingPage::navigate_to, this, [this](Page page) {
        if (page == Page::Signup) page_navigation(Page::Signup);
    });
    QObject::connect(main_->signup_page(), &App::Views::SignupPage::signup_requested,
                     api_, &Infrastructure::Http::ApiClient::do_signup);
    ...
    QObject::connect(api_, &Infrastructure::Http::ApiClient::signup_failed, main_->signup_page(),
    ...
    QObject::connect(api_, &Infrastructure::Http::ApiClient::login_failed, main_->login_page(),
    ...
    for (auto* form : {main_->client_page(), main_->business_settings_page(), main_->stock_settings_page(), main_->account_settings_page()}) {
        QObject::connect(form, &App::Views::ManagementForm::navigate_to, this, &AppController::page_navigation);
        QObject::connect(form, &App::Views::ManagementForm::submit_resource, api_, &Infrastructure::Http::ApiClient::save_resource);
    }
    QObject::connect(api_, &Infrastructure::Http::ApiClient::resource_saved, this, &AppController::resource_saved);
    QObject::connect(api_, &Infrastructure::Http::ApiClient::resource_save_failed, this, [this](const QString& message) {
...
void AppController::resource_saved(const QString& resource) {
    if (resource == "client") main_->client_page()->set_status("Client saved.");
    else if (resource == "business") main_->business_settings_page()->set_status("Business saved.");
    else if (resource == "stock") main_->stock_settings_page()->set_status("Stock item saved.");
    else main_->account_settings_page()->set_status("Account saved.");
}
